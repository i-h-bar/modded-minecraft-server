...
REQUIRED_VARS="MINECRAFT_HOST_DIR NGROK_AUTHTOKEN FORGE_VERSION MEMORY_ALLOCATION MODS_DIR"
ENV_FILE="./.env"
...
url=$(docker logs ngrok-tunnel | grep "url=" | awk -F'url=' '{print $2}' | sed 's/tcp:\/\///' | tr -d '\r')
